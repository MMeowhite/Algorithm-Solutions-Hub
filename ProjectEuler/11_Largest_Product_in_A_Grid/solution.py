def largest_product_in_a_grid(grid, window):
	"""
	Question: In the 20x20 grid below, four numbers along a diagonal line have been marked in red.
...
01 70 54 71 83 51 54 69 16 92 33 48 61 43 52 01 89 19 67 48
	...
	for i in range(row):
		for j in range(col):
			...
			if i + window <= row and j + window <= col:
				product = 1
				for k in range(window):
					product *= grid[i+k][j+k]
				if product > max_product:
				 	max_product = product
			# diagonal direction (accessory)
			if i + window <= row and j - window + 1 >= 0:
				product = 1
				for k in range(window):
					product *= grid[i+k][j-k]
				if product > max_product:
...
if __name__ == "__main__":
    ...
    window = 4  # 四个相邻数字
    print(largest_product_in_a_grid(grid, window))
